...
{
    static int lit = 0;
    //static int mask = 0x0200;
//    volatile int lit = (RPI_GetGpio()->GPLEV0) & mask;
...
//	int input = (RPI_GetGpio()->GPLEV0) & mask;
//	if (input == mask) {
		RPI_GetGpio()->GPSET0 = ((1 << 16) | (1 << 18));
        	lit = 0;
//	}
    }
    else
